...
# Find all records with pattern: digits + G
g_series = []

for book in books:
    legacy_id = book.get("legacy_id", "")
    
    # Match pattern: one or more digits followed by 'G'
    if legacy_id and re.match(r'^\d+G$', legacy_id, re.IGNORECASE):
        # Extract the numeric part
        numeric_part = legacy_id[:-1]  # Remove the 'G'
        try:
...
print(f"\n--- DIGITS+G PATTERN ANALYSIS ---")
print(f"Total records: {len(g_series)}")

if g_series:
    # Sort by numeric value
    g_series.sort(key=lambda x: x['numeric_value'])
    
    min_record = g_series[0]
    max_record = g_series[-1]
